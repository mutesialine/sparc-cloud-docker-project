...
- Build separate development and production images using one `Dockerfile`.
- Understand how Docker layer order speeds up builds with caching.
- Copy files between build stages using `COPY --from`.
- Run a multi-container application with Docker Compose.
- Configure containers with environment variables.
- Control container startup using health checks and `depends_on`.
- Keep data safe using Docker volumes.
- Sync local code changes with a running container for live development.
